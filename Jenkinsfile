            ...
            steps {
                sshagent(credentials: ['prod-ec2-ssh-key']) {
                    sh """
                        ...
                        scp docker-compose.prod.yml deploy/deploy.sh deploy/fetch-secrets.sh deploy/backup.sh deploy/restore.sh ${params.PROD_SSH_USER}@${params.PROD_HOST}:/opt/ecommerce/deploy/
